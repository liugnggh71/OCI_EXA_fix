...
do
  ...
  sleep 1
  ssh -i $HOME/cloud_agent_private_key.txt opc@${ip} sudo -u root chown oracle.dba /tmp/${1} < /dev/null
  sleep 1
  ssh -i $HOME/cloud_agent_private_key.txt opc@${ip} sudo -u oracle mkdir -p /home/oracle/dba_code/${subdir} < /dev/null
  sleep 1
  ssh -i $HOME/cloud_agent_private_key.txt opc@${ip} sudo -u root mv /tmp/${1} /home/oracle/dba_code/${1} < /dev/null
  sleep 1
